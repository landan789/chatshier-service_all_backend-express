...
class InvoiceVerifyBase{
    constructor(){
        this.param_xml_file = fs.readFileSync(__dirname + '/../../lib/ecpay_invoice/ECpayInvoice.xml').toString();
...
class InvoiceParamVerify extends InvoiceVerifyBase{
        ...
        if (params.constructor === Object){
            ...
            if (params['Print'] === '1'){
                ...
                if (params['CarruerNum'] !== '') {
                  throw new ECpayError.ECpayInvoiceRuleViolate(`[Print] can not be '1' when [CarruerNum] is not empty.`);
                }
            }
            ...
            if (params['TaxType'] === '2'){
              ...
              var vat_params_list = ['ItemCount', 'ItemAmount'];
                ...
                if (params['ItemTaxType'] !== ''){
                    throw new ECpayError.ECpayInvoiceRuleViolate(`[ItemTaxType] must be empty when [TaxType] is 2.`);
                }
            ...
            if (params['TaxType'] === '1'){
              ...
              var vat_params_list = ['ItemCount', 'ItemAmount'];
                if (params['ClearanceMark'] !== ''){
                    throw new ECpayError.ECpayInvoiceRuleViolate(`[ClearanceMark] must be empty when [TaxType] is 1.`);
                }
                if (params['ItemTaxType'] !== ''){
                    throw new ECpayError.ECpayInvoiceRuleViolate(`[ItemTaxType] must be empty when [TaxType] is 1.`);
                }
            ...
            if (params['TaxType'] === '3'){
              ...
              var vat_params_list = ['ItemCount', 'ItemAmount'];
                if (params['ClearanceMark'] !== ''){
                    throw new ECpayError.ECpayInvoiceRuleViolate(`[ClearanceMark] must be empty when [TaxType] is 3.`);
                }
                if (params['ItemTaxType'] !== ''){
                    throw new ECpayError.ECpayInvoiceRuleViolate(`[ItemTaxType] must be empty when [TaxType] is 3.`);
                }
            ...
            if (params['TaxType'] === '9'){
              ...
              var vat_params_list = ['ItemCount', 'ItemAmount', 'ItemTaxType'];
                if (!params['ItemTaxType'].includes('|')){
                    throw new ECpayError.ECpayInvoiceRuleViolate(`[ItemTaxType] must contain at lease one '|'.`);
                }
                if (params['ItemTaxType'] === ''){
                    throw new ECpayError.ECpayInvoiceRuleViolate(`[ItemTaxType] can not be empty when [TaxType] is 9.`);
                }
                // 當[ItemTaxType]含2選項的話[ClearanceMark]須為1或2
                if (params['ItemTaxType'].includes('2')){
                  if (params['ClearanceMark'] !== '1' && params['ClearanceMark'] !== '2'){
                    throw new ECpayError.ECpayInvoiceRuleViolate(`[ClearanceMark] has to be 1 or 2 when [ItemTaxType] has 2.`);
                  }
                }
            }
            ...
            if (params['vat'] === '0'){
              if (!params['ItemPrice'].includes('|')){
                ...
                vat_params.forEach(function (param_name){
                  ...
                  if (vat_cnt !== p_cnt){
                    throw new ECpayError.ECpayInvoiceRuleViolate(`Count of item info [${param_name}] (${p_cnt}) not match count from [ItemPrice] (${item_cnt}).`);
                  }
                });
        ...
        if (params.constructor === Object){
            ...
            if (params['DelayFlag'] === '1'){
              if (parseInt(params['DelayDay']) > 15 || parseInt(params['DelayDay']) < 1){
                throw new ECpayError.ECpayInvoiceRuleViolate(`[DelayDay] must be between 1 and 15 when [DelayFlag] is '1'.`);
              }
            } else if (params['DelayFlag'] === '2'){
              if (parseInt(params['DelayDay']) > 15 || parseInt(params['DelayDay']) < 0){
                throw new ECpayError.ECpayInvoiceRuleViolate(`[DelayDay] must be between 0 and 15 when [DelayFlag] is '2'.`);
              }
            }
            // [CarruerType]為'' or 1 時 : CarruerNum = '', [CarruerType]為2, CarruerNum = 固定長度為16且格式為2碼大小寫字母加上14碼數字。
            // [CarruerType]為3, 固定長度為8且格式為1碼斜線「/」加上7碼數字及大小寫字母組成
    ...
    verify_inv_issue_invalid_param(params){
        if (params.constructor === Object){
            ...
            if (params['Reason'] === ''){
                throw new ECpayError.ECpayInvoiceRuleViolate(`[Reason] is empty.`);
            }
