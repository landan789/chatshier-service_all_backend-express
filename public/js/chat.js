...
(function() {
    ...
    var SOCKET_SERVER_URL = window.CHATSHIER.URL.API.replace('..', window.location.origin) + SOCKET_NAMESPACE;
    ...
    var ticketTableCtrl = (function() {
        ...
        /** @type {Chatshier.Models.AppsTickets} */
        var appsTickets = {};
        var api = window.restfulAPI;
        ...
        TicketTableCtrl.prototype.loadTickets = function(appId, userId, platformUid) {
            ...
            return Promise.resolve().then(() => {
                if (!appsTickets[appId]) {
                    return api.appsTickets.findAll(appId, userId).then((resJson) => {
                        let _appsTickets = resJson.data;
                        appsTickets[appId] = { tickets: {} };
                        if (!(_appsTickets && _appsTickets[appId])) {
                            return appsTickets[appId].tickets;
                        }
                        Object.assign(appsTickets[appId].tickets, _appsTickets[appId].tickets);
                        return appsTickets[appId].tickets;
                    });
                }
                return appsTickets[appId].tickets;
        ...
        TicketTableCtrl.prototype.addTicket = function(appId) {
            ...
            if (!description) {
                ...
                return api.appsTickets.insert(appId, userId, newTicket).then(function(resJson) {
                    let _appsTickets = resJson.data;
                    if (!appsTickets[appId]) {
                        appsTickets[appId] = { tickets: {} };
                    }
        ...
        TicketTableCtrl.prototype.updateTicket = function(appId, ticketId) {
            ...
            return api.appsTickets.update(appId, ticketId, userId, ticket).then(function(resJson) {
                let _appsTickets = resJson.data;
                Object.assign(appsTickets[appId].tickets, _appsTickets[appId].tickets);
                let platformUid = appsTickets[appId].tickets[ticketId].platformUid;
        ...
        TicketTableCtrl.prototype.deleteTicket = function(appId, ticketId) {
            if (confirm('確認刪除表單？')) {
                return api.appsTickets.remove(appId, ticketId, userId).then(function(resJson) {
                    ...
                    var _appsTickets = resJson.data;
                    var platformUid = _appsTickets[appId].tickets[ticketId].platformUid;
                    delete appsTickets[appId].tickets[ticketId];
                    instance.loadTickets(appId, userId, platformUid);
                }).catch(function() {
    ...
    chatshierSocket.once('connect', () => bindingSocketEvents(chatshierSocket));
    ...
    function bindingSocketEvents(socket) {
        ...
        socket.on(SOCKET_EVENTS.EMIT_MESSAGE_TO_CLIENT, function(data) {
            ...
            var messagesFromSocket = socketBody.messages;
            ...
            var nextMessage = function(i) {
                if (i >= messagesFromSocket.length) {
                    return Promise.resolve();
                }

                var message = messagesFromSocket[i];
                appsChatrooms[appId].chatrooms[chatroomId].messages[message._id] = message;
                ...
                if (isNewChatroom) {
                    ...
                    createChatroomTab(appId, chatroomId, _platformUid, person, true);
                } else {
                    updateChatroomTab(appId, chatroomId, senderMsger); // update 客戶清單
    ...
    function initChatData(apps) {
        ...
        for (var appId in apps) {
            ...
            // 將聊天室的順序進行排序，預設將最後聊天時間排至最前頭
            let chatroomIds = Object.keys(chatrooms).sort((a, b) => {
                let chatroomA = chatrooms[a];
            ...
            for (let i in chatroomIds) {
                ...
                createChatroomTab(appId, chatroomId, _platformUid, person, false);
            }
        }
    ...
    function generateMessageHtml(srcHtml, message, messager, messagerSelf, appType, isSearched) {
    ...
    function createChatroomTab(appId, chatroomId, platformUid, person, shouldPrepend) {
        var app = apps[appId];
        var chatroom = appsChatrooms[appId].chatrooms[chatroomId];
        ...
        if (isGroupChatroom) {
            person.photo = LINE === app.type ? LOGOS[LINE_GROUP] : 'image/group.png';
            person.name = chatroom.name || DEFAULT_CHATROOM_NAME;
        }
        var chatroomItemHtml = (
            '<li class="text-light nested list-group-item tablinks" ' + 'app-id="' + appId + '" chatroom-id="' + chatroomId + '" ' + (!isGroupChatroom ? 'platform-uid="' + platformUid + '"' : '') + ' app-type="' + app.type + '">' +
                '<img class="app-icon consumer-photo" src="' + person.photo + '" />' +
                '<span class="app-name">' + person.name + '</span>' +
                '<span class="unread-msg badge badge-pill ml-auto bg-warning' + (!messagerSelf.unRead ? ' d-none' : '') + '">' + (messagerSelf.unRead > 99 ? '99+' : ('' + messagerSelf.unRead)) + '</span>' +
            '</li>'
        );
        ...
        shouldPrepend ? $chatroomCollapse.prepend(chatroomItemHtml) : $chatroomCollapse.append(chatroomItemHtml);
        ...
        $consumerPhoto.one('error', function() {
            ...
            $consumerPhotos.prop('src', '/image/user_large.png');
        ...
        if (messagerSelf.unRead) {
            let $unreadCollapse = $ctrlPanelChatroomCollapse.find('.collapse.unread');
            shouldPrepend ? $unreadCollapse.prepend(chatroomItemHtml) : $unreadCollapse.append(chatroomItemHtml);
        }
        ...
        if (CHATSHIER !== app.type) {
            ...
            if (assigneeIds.indexOf(userId) >= 0) {
                let $assignedCollapse = $ctrlPanelChatroomCollapse.find('.collapse.assigned');
                shouldPrepend ? $assignedCollapse.prepend(chatroomItemHtml) : $assignedCollapse.append(chatroomItemHtml);
            } else {
                let $unassignedCollapse = $ctrlPanelChatroomCollapse.find('.collapse.unassigned');
                shouldPrepend ? $unassignedCollapse.prepend(chatroomItemHtml) : $unassignedCollapse.append(chatroomItemHtml);
            }
        }
    }
    ...
    function generateTicketHtml(appId, chatroomId, platformUid) {
        var ticketPanelHtml = (
    ...
    function userClickTablink() {
        ...
        var $chatContent = $(
            '<div class="chat-content shown" app-id="' + appId + '" chatroom-id="' + chatroomId + '">' +
        ...
        let app = apps[appId];
        let person = consumers[platformUid];
        if (app && CHATSHIER === app.type) {
            person = Object.assign({}, users[userId]);
            person.photo = LOGOS[app.type];
        ...
        var $profileGroup = $(generateProfileHtml(appId, chatroomId, platformUid, person));
        $profileWrapper.empty().append($profileGroup);

        var $profileToggle = $('.toolbar #profileToggle').removeClass('d-none');
        ...
        if (isGroupChatroom) {
            ...
            var $ticketGroup = $(generateTicketHtml(appId, chatroomId, platformUid));
            $ticketWrapper.append($ticketGroup);
    ...
    function updateChatroomTab(appId, chatroomId, messager) {
        ...
        if (messager && messager.platformUid && !chatroom.platformGroupId) {
            ...
            var consumer = consumers[platformUid];
            if (consumer) {
                let $appIcon = $chatroomTablinks.find('.app-icon');
                if (consumer.photo && consumer.photo !== $appIcon.first().attr('src')) {
                    $appIcon.attr('src', consumer.photo);
                }

                let displayName = (messagerSelf.namings && messagerSelf.namings[platformUid]) || (messager.namings && messager.namings[platformUid]) || consumer.name;
                let $appName = $chatroomTablinks.find('.app-name');
                if (displayName && displayName !== $appName.first().text()) {
